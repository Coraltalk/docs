...
### Regenerate questions with instructions

On the questions step, open the **Regenerate** panel to refine the question set in place with a natural-language prompt. Use it when the first pass is close but not quite right — for example, to shift difficulty, change focus, or trim the list.

<Steps>
  <Step title="Open the Regenerate panel">
    On the questions step, click **Regenerate** to expand the instruction panel.
  </Step>

  <Step title="Lock questions you want to keep">
    Click the lock icon on any question to pin it. Locked questions are excluded from regeneration and remain exactly as written.
  </Step>

  <Step title="Describe the change">
    Enter a short instruction in the input. Examples:

    ```text
    Make them harder
    Focus more on photosynthesis
    Add two questions about real-world applications
    Remove 3 questions
    ```
  </Step>

  <Step title="Update all questions">
    Click **Update all questions** (or press Enter) to regenerate unlocked questions based on your instruction.
  </Step>
</Steps>

<Info>
Published assignments open with all questions locked by default, so existing wording is preserved until you explicitly unlock and regenerate.
</Info>

<Tip>
Leave the instruction empty to regenerate unlocked questions using the original source materials and difficulty settings.
</Tip>

If every question is locked, unlock at least one before regenerating — Coral needs a slot to write into.
